...
var $author$project$Main$GetYamlData = {$: 'GetYamlData'};
...
var $author$project$Main$Item = F3(
	function (name, description, link) {
		return {description: description, link: link, name: name};
	});
		...
		switch (msg.$) {
			...
			case 'GetYamlData':
				return _Utils_Tuple2(
					...
					$elm$http$Http$get(
						{
							expect: $elm$http$Http$expectString($author$project$Main$GotYamlData),
							url: '/data.yml'
						}));
			case 'GotYamlData':
				var res = msg.a;
